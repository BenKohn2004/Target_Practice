...
unsigned long int Target_Debounce = 1;  // The Debounce time for the Target Line
bool display_ready_flag = 0;            // Displays Ready for OLED screen so that it only sends the data once.
bool display_go_flag = 0;               // Displays Go for OLED screen so that it only sends the data once.
...
void iterate_mode() {
  ...
  if (weapon_mode == 0) {         // Displays that Epee is selected
    ...
    Target_Debounce = 100;        // Sets Target Debounce Time for Saber, Measured in Micro for Saber as opposed to Millis()
...
void saber(int target_number) {
  ...
  if ((Target_State == LOW) && (Target_State_Prev == HIGH)) {  // Current Target State is Hit but Previous was not, then Falling Edge
    Target_Timer = micros();                                   // Starts the Target Timer
                                                               // Target_Timer = millis();                                   // Starts the Target Timer
  }

  if (micros() > (Target_Timer + Target_Debounce) && Target_State == LOW) {  // Checks the Debounce Timing to turn on the LED
...
void timed_target() {
  ...
  for (int i = 0; i < 3; i++) {
    ...
    display_ready_flag = 1;       //  Resets the Ready flag
    display_go_flag = 1;          // Resets the Go flag
    ...
    while (millis() < start_loop + randomtime[i] + start_loop_delay && reaction_loop_flag == 1)  // While loop will continue until loop time out or reaction loop flag is changed
    {
      ...
      if (millis() < start_loop + randomtime[i] && LED_Light_Flag == 0 && display_ready_flag == 1) {  // States Ready on the OLED prior to the Red Light
        display_ready_flag = 0;                                                                       // Allows Ready to displayed only once per iteration                                                                  // Turns off the Ready flag for this iteration
      ...
      if (millis() > start_loop + randomtime[i] && LED_Light_Flag == 0 && display_go_flag == 1) {  // Starts by running the Red Indication to Hit
        if (weapon_mode == 2) {                                                                    //
        ...
        display_go_flag = 0;     // Allows Go to displayed only once per iteration
